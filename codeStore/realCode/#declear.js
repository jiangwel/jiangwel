import {groupColor,windowsIdArray,currentWindowsId,variable1,variable2,variable3,sourseTime,hour,minute,second,time} from '../../js/background.js';
function Variable(clock,tabsIdList,matherWebId,intoGroupTabsNumbers,groupId,motherWebExist,isMomWeb,intoGroupTabId,currentId,groupTabsList) {
    this.clock = clock; //0:Does not exist group 1:Exist group
    this.tabsIdList = tabsIdList; //Tab's id where in the browser. 增
    this.matherWebId = matherWebId; //Mather wed's id
    this.intoGroupTabsNumbers = intoGroupTabsNumbers; //Tab's number that enterd group  
    this.groupId = groupId; //Exist group's id
    this.motherWebExist = motherWebExist; //Is mother web exist?
    this.isMomWeb = isMomWeb; //Is Mother web?
    this.intoGroupTabId = intoGroupTabId; //Will enter group what tabid
    this.currentId = currentId;
    this.groupTabsList = groupTabsList;
  }
/*
...
function addWindowsIdIntoArray(id) {
  ...
  */
  ...
  function tabFlow(object, tabid) {
      ...
      chrome.storage.local.set(object);
      console.log("当前标签的ID:", object.currentId,"之前所有标签页:",object.tabsIdList, time);
  ...
  function addIdToList(id, object) {
    //3.把当前的标签的id加入标签id列表 
      if (!object.tabsIdList.includes(id) && !object.groupTabsList.includes(id)) {
        object.tabsIdList.push(id);//增
        chrome.storage.local.set(object,()=>{
          console.log("实时标签页ID列表:", object.tabsIdList, time);
        })
      }
      if (object.tabsIdList.length >= 6){
        return true;
      } else {
          return false;
      }
      ...
      if (object.isMomWeb) {
        ...
        if (object.motherWebExist) {
            ...
            intoGroupTabId = object.matherWebId;
              ...
              object.matherWebId = object.tabsIdList[index];//查
              chrome.storage.local.set(object);
            //不存在其他母网页
        } else {
            //把除母网页之外最旧的标签放入组内
        ...
        //最旧的网页不属于母网页
          ...
          intoGroupTabId = object.tabsIdList[index];//查
      }
  ...
  */
  function groupAbout(index, object) {
      var tabId;
      var url;
      tabId = object.tabsIdList[index];//查
      ...
      chrome.tabs.get(tabId, function (tab) {
          ...
          if (isSuccessSearch(tab.url)) {
            ...
            object.tabsIdList.remove(object.intoGroupTabId);//删
